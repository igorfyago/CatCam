@echo off
REM Build ONLY CatCamAudio.exe (endpoint rename/hide helper for the installer).
setlocal
cd /d %~dp0
set VCVARS=
...
cl /nologo /std:c++17 /EHsc /O2 %DEFS% %INCS% CatCamAudio.cpp /Fe:CatCamAudio.exe ^
   /link %LIBDIRS% ole32.lib propsys.lib advapi32.lib || exit /b 1
echo === AUDIO DONE ===
endlocal
