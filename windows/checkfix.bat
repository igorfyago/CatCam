@echo off
REM Compile-check ONLY: builds both binaries under scratch names so the
REM running (locked) CatCamHost.exe / CatCamTray.exe are never touched.
REM Verifies the sources before the elevated swap. Delete freely.
cd /d %~dp0
set VCVARS=
...
if not defined SDK_VER (echo ERROR: Windows SDK not found & exit /b 1)
...
cl /nologo /std:c++17 /EHsc %DEFS% %INCS% CatCamHost.cpp ^
   ...
   /link %LIBDIRS% mf.lib mfplat.lib mfuuid.lib mfsensorgroup.lib ole32.lib ws2_32.lib || exit /b 1

echo --- tray ---
cl /nologo /std:c++17 /EHsc /O2 %DEFS% %INCS% CatCamTray.cpp ^
   /Fo:checkfix_tray.obj /Fe:checkfix_tray.exe ^
   /link %LIBDIRS% /MANIFESTUAC:NO /MANIFEST:EMBED /MANIFESTINPUT:CatCamTray.manifest ^
